# demoapp Pipeline Configuration
...
```
./fly sp -t <YOUR_TARGET_NAME> configure -c pipeline.yml -p demoapp --load-vars-from=credentials.yml
...
```
./fly -t <YOUR_TARGET_NAME> unpause-pipeline -p demoapp
...
```
<YOUR_CONCOURSE_URL>/pipelines/demoapp
```
...
```
./fly -t <YOUR_TARGET_NAME> destroy-pipeline -p demoapp
```